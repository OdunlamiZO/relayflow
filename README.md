...
- [x] Contact custom fields — per-workspace field schema (key/label/description) plus four reserved fields (`displayName`, `phone`, `email`, `country`) that can't be redefined. Reserved values auto-derive from existing data (e.g. phone from a WhatsApp identity) when not explicitly set, editable manually, by the AI agent (gap-filling only — never overwrites a value already in place), or by a workflow's **Set Contact Field** node (always overwrites, like a manual edit). Gated by the `CONTACT_FIELDS_WRITE` permission.
...
- [x] Workspace member permissions — owners manage members, transfer ownership, and grant granular access for inbox, contacts (including contact field schema/values), workflows, channels, API keys, and webhooks.
